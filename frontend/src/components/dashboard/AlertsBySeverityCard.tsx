    ...
    <div className="flex flex-col rounded-xl border-2 border-gray-700 bg-gray-100 p-5">
                ...
                >
                  {data.map((entry, idx) => (
                    <Cell 
                      ...
                      fill={entry.fill} 
                      stroke="transparent" 
                      onClick={() => navigate(`/history?severity=${entry.name.toLowerCase()}&status=open`)}
      ...
      <div className="mt-3 flex justify-center flex-wrap gap-x-4 gap-y-1.5 text-xs">
        {data.map((t) => (
          <span 
            key={t.name} 
            className="capitalize text-sm flex items-center gap-1.5 text-gray-700 cursor-pointer hover:underline"
            onClick={() => navigate(`/history?severity=${t.name.toLowerCase()}&status=open`)}
          >
